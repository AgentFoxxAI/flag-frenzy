...
const DEFAULT_SETTINGS={minutes:2,level:0,bonus:"capital",style:"classic",cont:"ALL",sound:true,speak:false,practiceTimed:false,practiceAuto:true,flagset:"modern",spotSize:"m",blitzCount:20,localCount:2};
...
const S={
 mode:"two",
 players:[{name:"",avatar:{type:"hist",id:"einstein"}},{name:"",avatar:{type:"hist",id:"cleopatra"}},{name:"",avatar:{type:"hist",id:"caesar"}},{name:"",avatar:{type:"hist",id:"amelia"}}],
...
function renderPlayers(){
 const box=$("players");const n=S.mode==="two"?S.settings.localCount:1;box.classList.toggle("solo",n===1);
 $("countRow").hidden=S.mode!=="two";document.querySelectorAll("#countChips .chip").forEach(c=>c.classList.toggle("on",+c.dataset.n===S.settings.localCount));
...
document.querySelectorAll("#countChips .chip").forEach(c=>c.onclick=()=>{S.settings.localCount=+c.dataset.n;saveSettings();renderHome();sfx.tap()});
...
$("btnSaveSettings").onclick=()=>{saveSettings();savePlayers();S.players.slice(0,S.mode==="two"?S.settings.localCount:1).forEach((_,i)=>saveProfile(i,false));store.set("profiles",S.profiles);const t=new Date();$("saveStatus").textContent=`Saved ✓ ${t.toLocaleTimeString([],{hour:"numeric",minute:"2-digit"})}`;flash("Settings and players saved ✓");sfx.correct();renderPlayers()};
...
$("btnStart").onclick=()=>{
 ...
 S.order=S.mode==="two"?Array.from({length:S.settings.localCount},(_,i)=>i):[0];S.cur=0;S.results=[];
...
function endRound(){
 ...
 if(R.blitz){
  ...
  $("btnSumNext").textContent=hasBonus?"🎁 Bonus round ➜":(S.mode==="two"&&S.cur<S.order.length-1?"Next player ➜":(S.mode==="practice"?"Done ➜":"See results ➜"));
 }
 $("btnSumNext").disabled=false;
 if(S.mode==="online"&&NET){NET.done=true;NET.doneAt=Date.now();netSendFinal()}
 showScreen("s-summary");
 if(S.mode==="online"&&NET)netCheckResults();
}
...
function showResults(){
 ...
 if(S.mode==="two"){
  const entries=S.results.map(r=>({name:playerName(r.player),avatar:S.players[r.player].avatar,score:r.score,correct:r.correct,bonus:r.bonus}));
  showRanking(entries,true);
  $("resRankLabel").textContent="Leaderboard: "+S.results.map(r=>`${playerName(r.player)} #${rank(r)}`).join(" · ")+` · ${L.emoji} ${L.name}`;
 }else{
...
function showRanking(entries,countWin){
 entries.sort((x,y)=>y.score-x.score);const top=entries[0].score;const winners=entries.filter(e=>e.score===top);
 if(winners.length===1){$("resTitle").textContent=`🏆 ${winners[0].name} wins!`;if(countWin===true||(countWin==="me"&&winners[0].me)){const wins=store.get("wins",{});wins[winners[0].name]=(wins[winners[0].name]||0)+1;store.set("wins",wins)}}
 else $("resTitle").textContent=`🤝 Tie: ${winners.map(w=>w.name).join(" & ")}!`;
 let rk=0,prev=null;
 $("resBody").innerHTML=entries.map((e,i)=>{if(e.score!==prev){rk=i+1;prev=e.score}const medal=["🥇","🥈","🥉"][rk-1]||rk+".";
  return `<div class="rk ${rk===1?"first":""} ${e.me?"me":""}"><span class="rkn">${medal}</span>${avatarHTML(e.avatar,"sm")}<div class="rkname"><b>${esc(e.name)}</b>${e.me?' <span class="muted">(you)</span>':""}<div class="muted">${e.correct||0} right${e.bonus?` · +${e.bonus} bonus`:""}${e.left?" · left early":""}${e.playing?" · still playing":""}</div></div><div class="rksc">${e.score}</div></div>`}).join("");
}
function confetti(){
...
function myMeta(){return {...whoAmI(),started:NET.started,seed:NET.seed,roster:NET.host?NET.roster:null,cfg:NET.cfg,score:R?R.score:0,correct:R?R.correct:0,wrong:R?R.wrong:0,n:R?R.seen.length:0,done:NET.done,ts:Date.now()}}
function netSubscribe(){
 ...
 ch.on("broadcast",{event:"state"},m=>{const p=m&&m.payload;if(!NET||!p||!p.id||p.id===NET.id)return;const cur=NET.lives[p.id];if(!cur||(p.ts||0)>=(cur.p.ts||0)){NET.lives[p.id]={p,at:Date.now()};netOnSync()}});
 ch.on("broadcast",{event:"needav"},m=>{const p=m&&m.payload;if(!NET||!p||p.id===NET.id||p.want!==NET.id||!NET.myPic)return;ch.send({type:"broadcast",event:"avatar",payload:{id:NET.id,data:NET.myPic}})});
 ch.on("broadcast",{event:"avatar"},m=>{const p=m&&m.payload;if(!NET||!p||p.id===NET.id)return;if(typeof p.data==="string"&&p.data.startsWith("data:image/jpeg;base64,")&&p.data.length<60000){NET.pics[p.id]=p.data;renderLobby();renderOppHud()}});
 ch.subscribe(async status=>{
  if(!NET||NET.ch!==ch)return;
  if(status==="SUBSCRIBED"){NET.retries=0;try{await ch.track({...whoAmI(),ts:NET.joinTs})}catch(e){}netSend();if(!NET.started&&!NET.shown){NET.shown=true;showLobby()}}
...
async function netJoin(code,host,calling){
 ...
 NET={id,code,host,started:false,seed:null,cfg:host?makeCfg():null,done:false,peers:[],known:{},lives:{},pics:{},roster:null,asked:{},invites:{},joinTs:Date.now(),sb:SB,ch:null,calling:calling||null,hb:null,retries:0,shown:false,leaving:false};
...
const MAX_PLAYERS=4;
// Everyone else in the room: identity from presence, numbers from each player's newest broadcast.
function netPeers(){if(!NET||!NET.ch)return [];const who={};
 try{const st=NET.ch.presenceState();for(const k in st){if(k===NET.id)continue;for(const m of st[k]){if(m&&(!who[k]||(m.ts||0)>(who[k].ts||0)))who[k]=m}}}catch(e){}
 const ids=new Set(Object.keys(who));for(const id in NET.lives){if(Date.now()-NET.lives[id].at<15000)ids.add(id)}
 if(NET.roster)for(const id of NET.roster){if(id!==NET.id)ids.add(id)}
 let list=[...ids].map(id=>{const l=NET.lives[id];const here=!!who[id]||!!(l&&Date.now()-l.at<15000);
  const merged={...(NET.known[id]||{}),...(who[id]||{}),...(l?l.p:{}),id,joinTs:(who[id]&&who[id].ts)||(NET.known[id]&&NET.known[id].joinTs)||0,here};NET.known[id]=merged;return merged});
 if(NET.roster)list=list.filter(p=>NET.roster.includes(p.id));
 return list.sort((x,y)=>(x.joinTs-y.joinTs)||(x.id<y.id?-1:1)).slice(0,NET.roster?MAX_PLAYERS:MAX_PLAYERS+2)}
function netOnSync(){
 if(!NET)return;const peers=netPeers();NET.peers=peers;
 // room is capped at four: the newest arrival steps back out
 if(!NET.started&&!NET.host&&peers.filter(p=>p.here&&p.joinTs&&p.joinTs<NET.joinTs).length>=MAX_PLAYERS){netLeave();renderHome();showScreen("s-home");flash("That room is full (4 players max)");return}
 if(peers.length&&!NET.greeted){NET.greeted=true;setTimeout(netSend,300)}
 for(const p of peers){if(p.cav&&!NET.pics[p.id]&&(!NET.asked[p.id]||Date.now()-NET.asked[p.id]>3000)){NET.asked[p.id]=Date.now();try{NET.ch.send({type:"broadcast",event:"needav",payload:{id:NET.id,want:p.id}})}catch(e){}}}
 renderLobby();renderOppHud();
 const host=peers.find(p=>p.host&&p.started&&p.seed!=null);
 if(!NET.started&&!NET.host&&host){
  if(Array.isArray(host.roster)&&!host.roster.includes(NET.id)){netLeave();renderHome();showScreen("s-home");flash("That game already started. Ask them for a new room.");return}
  NET.started=true;NET.seed=host.seed;NET.cfg=host.cfg;NET.roster=host.roster||null;Object.assign(S.settings,host.cfg||{});renderHome();netStartRound()}
 if(NET.done)netCheckResults();
}
function avatarFor(m){if(m&&m.cav&&NET&&NET.pics[m.id])return {type:"custom",data:NET.pics[m.id]};return m&&m.av&&AVATARS.find(a=>a.id===m.av)?{type:"hist",id:m.av}:{type:"hist",id:"amelia"}}
function showLobby(){$("lobbyCode").textContent=NET.code;$("btnLobbyStart").hidden=!NET.host;$("lobbyHint").textContent=NET.calling?`Calling ${NET.calling.name}… They need Flag Frenzy open on their screen. Or just tell them the code.`:(NET.host?"Tell the other players this code. They tap Online Duel → Join a room. Up to 4 can play.":"You're in! Waiting for the host to start.");renderLobby();showScreen("s-lobby");if(NET.calling)pingFriend(NET.calling)}
function renderLobby(){
 if(!NET||NET.started)return;const peers=(NET.peers||[]).filter(p=>p.here).slice(0,MAX_PLAYERS-1);
 let h=`<div class="lobbyp">${avatarHTML(S.players[0].avatar)}<span>${esc(playerName(0))} (you)</span></div>`;
 h+=peers.map(p=>{const canF=p.did&&p.did!==DID&&!S.friends.some(f=>f.did===p.did);return `<div class="lobbyp">${avatarHTML(avatarFor(p))}<span>${esc(p.name||"Player")}${p.host?" 👑":""}</span>${canF?`<button class="btn sm sun" data-savefriend="${esc(p.id)}">⭐ Save friend</button>`:""}</div>`}).join("");
 for(let i=peers.length+1;i<MAX_PLAYERS;i++)h+=`<div class="lobbyp empty">${avatarHTML({type:"hist",id:"amelia"},"locked")}<span>Open seat</span></div>`;
 $("lobbyPlayers").innerHTML=h;
 $("lobbyPlayers").querySelectorAll("[data-savefriend]").forEach(b=>b.onclick=()=>{saveFriend(NET.known[b.dataset.savefriend]);renderLobby()});
 $("btnLobbyStart").disabled=!peers.length;$("btnLobbyStart").textContent=peers.length?`▶ Start with ${peers.length+1} players`:"▶ Start";
 const hostPeer=peers.find(p=>p.host);const hostCfg=NET.host?NET.cfg:(hostPeer?hostPeer.cfg:null);$("lobbyCfg").textContent=cfgSummary(hostCfg);$("lobbyCfgNote").textContent=NET.host?"To change these, leave the room, adjust the settings, and create a new room.":"These come from the host's screen. Your own settings are not used in this game.";
 const inv=S.friends.filter(f=>!peers.some(p=>p.did===f.did));$("lobbyInvite").hidden=!inv.length||peers.length>=MAX_PLAYERS-1;
 $("lobbyInviteChips").innerHTML=inv.map(f=>`<button class="chip ${NET.invites[f.did]?"on":""}" data-inv="${esc(f.did)}">${NET.invites[f.did]?"📣 Calling":"➕"} ${esc(f.name)}</button>`).join("");
 $("lobbyInviteChips").querySelectorAll("[data-inv]").forEach(b=>b.onclick=()=>{pingFriend(S.friends.find(f=>f.did===b.dataset.inv));renderLobby()});
 $("lobbyStatus").textContent=peers.length?`${peers.length+1} of ${MAX_PLAYERS} players are here. `+(NET.host?"Start whenever you're ready, or wait for more.":"Waiting for the host to start…"):(NET.host?"Waiting for players to join…":"Nobody else is here yet. Double-check the code.");
}
$("btnLobbyStart").onclick=async()=>{if(!NET||!NET.host)return;const peers=(NET.peers||[]).filter(p=>p.here).slice(0,MAX_PLAYERS-1);if(!peers.length)return;NET.seed=Math.floor(Math.random()*2147483647);NET.cfg=makeCfg();NET.roster=[NET.id,...peers.map(p=>p.id)];NET.started=true;let k=0;const announce=()=>{if(!NET||!NET.started)return;netSend();if(++k<6)setTimeout(announce,700)};announce();netStartRound()};
...
function netLeave(){if(!NET)return;NET.leaving=true;clearInterval(NET.hb);try{for(const d in NET.invites){clearInterval(NET.invites[d].t);NET.sb.removeChannel(NET.invites[d].ch)}NET.ch.untrack();NET.sb.removeChannel(NET.ch)}catch(e){}NET=null;$("oppHud").hidden=true}
const peerGone=p=>!p.here&&!p.done;
function renderOppHud(){
 const box=$("oppHud");if(!NET||!NET.started){box.hidden=true;return}
 const peers=NET.peers||[];box.hidden=false;const best=Math.max(R?R.score:0,...peers.map(p=>p.score||0));
 box.innerHTML=peers.length?peers.map(p=>`<div class="opp ${(p.score||0)===best&&best>0?"leader":""}">${avatarHTML(avatarFor(p))}<div class="oppname">${esc(p.name||"Player")}</div><div class="oppinfo">${peerGone(p)?"left the game":(p.done?"finished ✓":`${p.n||0} flags · ${p.correct||0} right`)}</div><div class="oppscore">${p.score||0}</div></div>`).join(""):`<div class="opp"><div class="oppinfo">Connecting to the other players…</div></div>`;
}
function netCheckResults(){
 if(!NET||!NET.done||!document.getElementById("s-summary").classList.contains("on"))return;
 const waiting=(NET.peers||[]).filter(p=>!p.done&&p.here);
 if(!waiting.length){$("btnSumNext").disabled=false;$("btnSumNext").textContent="See results ➜"}
 else if(Date.now()-(NET.doneAt||0)>20000){$("btnSumNext").disabled=false;$("btnSumNext").textContent=`See results without ${waiting.map(p=>p.name||"a player").join(" and ")} ➜`}
 else{$("btnSumNext").disabled=true;$("btnSumNext").textContent=`⏳ Waiting for ${waiting.map(p=>p.name||"a player").join(" and ")}…`}
}
function showResultsOnline(){
 const peers=NET?netPeers():[];const mine=S.results[0];const L=LEVELS[S.settings.level];
 const entries=[{name:playerName(0),avatar:S.players[0].avatar,score:mine.score,correct:mine.correct,me:true},...peers.map(p=>({name:p.name||"Player",avatar:avatarFor(p),score:p.score||0,correct:p.correct||0,left:peerGone(p),playing:!p.done&&p.here,did:p.did,pid:p.id}))];
 const hof=store.get("hof",[]);hof.push({n:playerName(0),a:S.players[0].avatar,s:mine.score,l:S.settings.level,ln:L.emoji+" "+L.name,m:S.settings.minutes,d:new Date().toISOString().slice(0,10),mode:"online"});hof.sort((a,b)=>b.s-a.s);store.set("hof",hof.slice(0,30));
 showRanking(entries,"me");
 $("resRankLabel").textContent=`🌐 Online game · ${entries.length} players · ${L.emoji} ${L.name} · ${fmt(S.settings.minutes*60)}`;
 let unlockHTML="";if(S.unlocked<AVATARS.length){const a=AVATARS[S.unlocked];S.unlocked++;store.set("unlocked",S.unlocked);unlockHTML=`<div class="unlock pop">${avatarHTML({type:"hist",id:a.id},"sm")}<div><b>🎉 New Legend unlocked: ${a.name}!</b><p>${a.bio}</p></div></div>`}
 $("resUnlock").innerHTML=unlockHTML;
 entries.filter(e=>e.did&&e.did!==DID&&!S.friends.some(f=>f.did===e.did)).forEach(e=>{const b=document.createElement("button");b.className="btn sm sun";b.style.margin="12px 6px 0 0";b.textContent=`⭐ Save ${e.name} as a friend`;const rec={did:e.did,name:e.name,av:(NET&&NET.known[e.pid]||{}).av||null};b.onclick=()=>{saveFriend(rec);b.remove()};$("resUnlock").appendChild(b)});
 showScreen("s-results");sfx.fanfare();confetti();netLeave();
}
...
function onChallenge(p){const f=S.friends.find(x=>x.did===p.fromDid);if(!f||!/^[A-Z0-9]{4}$/.test(p.code||"")||p.code===lastChallenge)return;
 ...
 askConfirm(`🌐 ${f.name} wants a duel!`,"Same flags, same time, each on your own screen.","Accept ⚔️","Not now",()=>{S.mode="online";store.set("mode","online");renderHome();netJoin(p.code,false)})}
async function challengeFriend(f){if(!f)return;document.querySelectorAll("#players input").forEach((inp,i)=>S.players[i].name=inp.value);saveProfile(0,false);savePlayers();sfx.tap();await netJoin(makeCode(),true,f)}
function pingFriend(f){if(!f||!NET||NET.started||NET.invites[f.did])return;const code=NET.code;const ch=SB.channel("ff-inbox-"+f.did);const inv={ch,t:null,n:0};NET.invites[f.did]=inv;
 ch.subscribe(st=>{if(st!=="SUBSCRIBED"||!NET||NET.code!==code)return;
  const ping=()=>{if(!NET||NET.code!==code||NET.started||inv.n++>45||(NET.peers||[]).some(p=>p.did===f.did)){clearInterval(inv.t);return}ch.send({type:"broadcast",event:"challenge",payload:{fromDid:DID,fromName:playerName(0),code}})};
  ping();inv.t=setInterval(ping,2000)})}
...
function boot(hot){
 ...
 const ps=store.get("players",null);if(Array.isArray(ps))ps.slice(0,4).forEach((p,i)=>{if(p&&p.avatar)S.players[i]={name:p.name||"",avatar:p.avatar}});
 if(hot&&hot.settings){Object.assign(S.settings,hot.settings);if(hot.mode)S.mode=hot.mode;if(Array.isArray(hot.players))hot.players.slice(0,4).forEach((p,i)=>{if(p&&p.avatar)S.players[i]=p})}
 ...
 if(![10,20,30,50].includes(S.settings.blitzCount))S.settings.blitzCount=20;if(![2,3,4].includes(S.settings.localCount))S.settings.localCount=2;if(!["two","solo","practice","blitz","online"].includes(S.mode))S.mode="two";
